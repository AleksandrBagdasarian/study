...
# Задаем рандомное присвоение первого числа из диапазона с 3 до 20
...
def сhristopher_jr():
    ...
    for i in range(first_digit):
        ...
        for j in range(i + 1, first_digit - 1):
            # Проверка кратности - деление без остатка суммы 2х последовательных элементов списка
            if first_digit % (selection_list[i] + selection_list[j]) != 0:
                # Возвращаем цикл в начало, если условие не соблюдено
                continue
            # Если условие соблюдено, добавляем соответствующие значения в к конечный список
            else:
